...
const cases = [
  ...
  {
    bot:'Atlas', name:'synthesizes the whole operation into one narrow fit',
    run:() => atlasTurn([
      U("I run a roofing company. We're booked six weeks out and don't need more install leads."),
      A('Then more leads are not the value case.'),
      U("My wife handles the phones, but service and warranty callbacks disappear when she's tied up. Half the other calls are junk."),
      A('That is an operating role, not a gap to erase.'),
      U('Put that together. What exactly would you do for us, and what would you leave alone?'),
    ]),
    checks:r => [
      ['valid answer', r._status === 200 && noAtlasHype(r)],
      ['does not manufacture demand', contains(r.say, /\bdo not use Atlas to create more demand\b/i)],
      ['protects the existing family desk', contains(r.say, /\bnot replace your family-run desk\b/i)],
      ['does not count noise as value', contains(r.say, /\bnot count junk calls\b/i)],
      ['selects the real uncovered lane', contains(r.say, /\bservice and warranty callbacks\b/i)],
      ['defines a watched proof gate', contains(r.say, /\bwatched mix of real and junk calls\b/i)],
      ['follow-up surface', r.screen === 'room:follow'],
    ],
  },
  {
    bot:'Atlas', name:'lets a correction replace the old workflow',
    run:() => atlasTurn([
      U('We miss calls and I thought Atlas should book every estimate.'),
      A('Booking can be one Atlas workflow.'),
      U('Actually, keep booking with our dispatcher. The only lane I want help with is unanswered follow-up after a missed call.'),
      A('Understood.'),
      U('Based on that correction, what should you set up first?'),
    ]),
    checks:r => [
      ['valid answer', r._status === 200 && noAtlasHype(r)],
      ['latest correction wins', contains(r.say, /\bstart with Follow-up\b/i)],
      ['keeps the dispatcher on booking', contains(r.say, /\bkeep booking with your dispatcher\b/i)],
      ['does not silently return to booking automation', !contains(r.say, /\bstart with Booking\b/i)],
      ['follow-up surface', r.screen === 'room:follow'],
    ],
  },
  {
    bot:'Atlas', name:'makes its accumulated understanding inspectable',
    run:() => atlasTurn([
      U("We're a roofing company, booked out, and my wife runs the desk."),
      A('I will not treat a working desk as a gap.'),
      U('The one leak is service and warranty callbacks.'),
      A('That is the lane to examine.'),
      U('Before you recommend anything, tell me what you understand about our operation.'),
    ]),
    checks:r => [
      ['valid answer', r._status === 200 && noAtlasHype(r)],
      ['recalls the trade', contains(r.say, /\broofing\b/i)],
      ['recalls capacity', contains(r.say, /\bbooked out|at capacity\b/i)],
      ['recalls the current desk', contains(r.say, /\bfamily-run desk|wife runs the desk\b/i)],
      ['recalls the actual gap', contains(r.say, /\bservice and warranty callbacks\b/i)],
      ['makes correction easy', contains(r.say, /\bcorrect .* wrong|if I have .* wrong\b/i)],
    ],
  },
  {
  ...
  {
    bot:'Vesta', name:'turns a whole quote conversation into one exact next move',
    run:() => vestaTurn([
      ...
      A('Written clarity is not nitpicking.'),
      U('Put it together. What is my exact next move?'),
    ]),
    checks:r => [
      ['valid final frame', r._status === 200 && !r._error],
      ['asks for roof-specific comparable scope', contains(r.say, /\btear-off layers\b/i) && contains(r.say, /\bflashing\b/i) && contains(r.say, /\bdeck\b/i)],
      ['compares equivalent scopes rather than totals', contains(r.say, /\bcompare equivalent scopes, not totals\b/i)],
      ['does not equate detail with quality', contains(r.say, /\bmore legible, not automatically better\b/i)],
      ['removes the conflict shame', contains(r.say, /\bnormal decision hygiene, not confrontation\b/i)],
  ...
  },
  {
    ...
    run:() => vestaTurn([
      ...
      A('Repair versus replacement needs an inspection.'),
      U('Actually, the HVAC tech says it is repairable. I am not deciding repair versus replace now; I am comparing two repair proposals.'),
      A('Understood.'),
      U('Before you advise me, tell me what you understand so far.'),
    ]),
    checks:r => [
      ['valid final frame', r._status === 200 && !r._error],
      ['recalls the trade', contains(r.say, /\bHVAC\b/i)],
      ['latest repair decision wins', contains(r.say, /\btwo repair proposals\b/i)],
      ['does not revive replacement advice', !contains(r.say, /\bchoose between repair and replacement|repair-versus-replacement decision\b/i)],
      ['makes correction easy', contains(r.say, /\bcorrect .* wrong|if I have .* wrong\b/i)],
      ['no accidental match or fabricated price', !r.resolved && noVestaFabrication(r)],
    ],
  },
  {
    bot:'Vesta', name:'combines a mid-job change and power concern into an ordered recovery',
    run:() => vestaTurn([
      ...
      A('Reasonable written clarity is not nitpicking.'),
      U('Give me the exact order of what I should do next.'),
    ]),
    checks:r => [
      ['valid final frame', r._status === 200 && !r._error],
      ['starts with evidence', contains(r.say, /\bphotos or other evidence\b/i)],
      ['requires a revised scope and impact', contains(r.say, /\brevised scope\b/i) && contains(r.say, /\bprice and schedule\b/i)],
