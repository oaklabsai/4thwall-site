...
function vestaRecap(state){
  if (state.repairDecision?.value === 'repair_proposals') {
    ...
    return `You are comparing ${count}${state.trade?.value || 'equipment'} repair proposals after a technician said the furnace is repairable. Replacement is no longer the decision. If I have any of that wrong, correct it before I advise you.`;
  }
  const facts = [];
