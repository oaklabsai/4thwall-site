...
const firstHome = vestaDecisionRoute('I just bought my first house and do not know what kind of contractor I need.', []);
T('first-time homeowner gets shame-free observable-fact intake', firstHome?.mode==='fix' && /not supposed to know the trade/i.test(firstHome.say) && /what changed/i.test(firstHome.ask || '') && /unsafe/i.test(firstHome.ask || ''));
const identityConcern = vestaDecisionRoute('Are contractors ignoring me because I am a woman?', []);
T('identity concern avoids motive fiction and preserves agency', /cannot know their motive/i.test(identityConcern?.say || '') && /do not need to prove why/i.test(identityConcern?.say || '') && /move on/i.test(identityConcern?.say || ''));
const conflictConcern = vestaDecisionRoute('I do not want to be nitpicky or piss him off. He might walk.', []);
T('power-imbalance coaching makes written clarity legitimate', /written clarity is not nitpicking/i.test(conflictConcern?.say || '') && /what both sides agreed/i.test(conflictConcern?.say || ''));
const prehireSilence = vestaDecisionRoute('Nobody calls me back. Am I doing something wrong?', []);
T('pre-hire silence does not blame the homeowner', /not proof you asked wrong/i.test(prehireSilence?.say || '') && /operating evidence/i.test(prehireSilence?.say || '') && /fit for you to quote/i.test(prehireSilence?.say || ''));
const changeOrder = vestaDecisionRoute('They found more work midway and want more money. What should I do?', []);
T('mid-job change requires evidence and written approval', /photos or other evidence/i.test(changeOrder?.say || '') && /written change order before/i.test(changeOrder?.say || ''));
const contractCoach = vestaDecisionRoute('What should the contractor contract include before I sign?', []);
T('contract coaching makes scope, payment, changes and warranty legible', /exact scope and materials/i.test(contractCoach?.say || '') && /payment milestones tied to work/i.test(contractCoach?.say || '') && /change-order approval/i.test(contractCoach?.say || '') && /warranties/i.test(contractCoach?.say || ''));
const referenceCoach = vestaDecisionRoute('Should I trust the references a contractor gives me?', []);
T('reference coaching treats selected references as one signal', /selected by the contractor/i.test(referenceCoach?.say || '') && /one signal, not a guarantee/i.test(referenceCoach?.say || '') && /public records/i.test(referenceCoach?.say || ''));
const guaranteeCoach = vestaDecisionRoute('Can Vesta guarantee the contractor you recommend will do good work?', []);
